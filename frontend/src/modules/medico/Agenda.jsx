...
const Agenda = () => {
  const { cargarAgenda, formatearFecha, showMessage, loading } = useMedico();
  const [citas, setCitas] = useState([]);
  const [fecha, setFecha] = useState(new Date().toISOString().split('T')[0]);

  useEffect(() => {
    const fetchCitas = async () => {
      const citasData = await cargarAgenda(fecha);
      setCitas(citasData);
    };
    fetchCitas();
  }, [fecha, cargarAgenda]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#667eea] to-[#764ba2] text-white font-sans">
      <header className="flex justify-between items-center p-5 md:p-10 bg-white/10 backdrop-blur-md border-b border-white/20">
        ...
        <div className="flex gap-4">
          ...
          <Link to="/agenda" className="px-5 py-2 bg-white/40 rounded-full text-white hover:bg-white/50 hover:-translate-y-0.5 transition-all">Agenda</Link>
      ...
      <main className="p-10 max-w-6xl mx-auto">
        <h1 className="text-4xl font-light text-center mb-8 text-shadow-md">Agenda</h1>

        <div className="bg-white/95 text-gray-800 p-10 rounded-2xl shadow-2xl">
          <div className="mb-6">
            <label htmlFor="fechaCita" className="text-lg font-semibold">Seleccionar Fecha:</label>
            <input
              type="date"
              id="fechaCita"
              value={fecha}
          ...
          <h2 className="text-2xl font-semibold mb-6 border-b-2 border-gray-200 pb-2">Citas del Día</h2>
          {citas.length === 0 ? (
